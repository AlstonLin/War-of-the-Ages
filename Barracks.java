...
{   
        ...
        if (owner.getID() == ((Game)getWorld()).getUser().getID()){
            ...
            if (owner.getAge() == 1) { //Medieval Age
                ...
                list.add(new Button(machineGunButton){ //Spawns Machine Gun
                        ...
                        public String getHoverInfo() {
                            return "Food: " + MachineGun.FOOD_COST + " | Wood:" + MachineGun.WOOD_COST + " | Gold:" + MachineGun.GOLD_COST + " | Training time: " + MachineGun.SPAWN_TIME;
                        }
                    });
    ...
    public void spawnUnits() {
        if (currentSpawn != null) { //Currently spawning something
            ...
            if (owner.getAge() == 1) {
                ...
                } else if (currentSpawn instanceof MachineGun){
                    counter = MachineGun.SPAWN_TIME;
    ...
    public void spawnMachineGun() {
        //Cannot spawn more Units if max population is reached, if there are not enough resources
        if (owner.getMaxPopulation() > owner.getPopulation() && owner.getFood() >= MachineGun.FOOD_COST 
        && owner.getWood() >= MachineGun.WOOD_COST && owner.getGold() >= MachineGun.GOLD_COST) {
            //Consumes resources
            owner.changeFood((Game)getWorld(), -MachineGun.FOOD_COST);
            owner.changeWood((Game)getWorld(), -MachineGun.WOOD_COST);
            owner.changeGold((Game)getWorld(), -MachineGun.GOLD_COST);

            queue.add(new MachineGun(owner, spawnPoint, ((Game) getWorld()).getMap()));
            Game game = (Game)getWorld();
            if (game.isMultiplayer() && !game.isHosting()) { //Bug Fix for clients
                Unit.IDCounter--;
            }
    ...
    private void updateImage() {
        ...
        //For Machine Gun
        if (owner.getMaxPopulation() <= owner.getPopulation() || owner.getFood() < MachineGun.FOOD_COST || owner.getWood() < MachineGun.WOOD_COST 
        || owner.getGold() < MachineGun.GOLD_COST) {
            machineGunButton.setTransparency(100);
        } else {
            machineGunButton.setTransparency(255);
        }
