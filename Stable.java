...
public class Stable extends Building
{
    ...
    public LinkedList<Button> getSpecificButtons() {
        ...
        if (owner.getAge() == 1 && owner.getID() == ((Game)getWorld()).getUser().getID()) { //Medieval 
            ...
            //Adds buttons onto Actions Panel
            list.add(new Button(knightButton){ //Spawn Knight
                    public void click() {
                        spawnKnight();
                        updateButtonImage();
                    }
                    ...
                    public String getHoverInfo() {
                        return "Food: " + Knight.FOOD_COST + " | Wood:" + Knight.WOOD_COST + " | Gold:" + Knight.GOLD_COST + " | Training time: " + Knight.SPAWN_TIME;
                    }
                });
            ...
            list.add(new Button(mountedArcherButton){ //Spawn Mounted Archer
                    public void click() {
                        spawnMountedArcher();
                        updateButtonImage();
                    }
                    ...
                    public String getHoverInfo() {
                        return "Food: " + MountedArcher.FOOD_COST + " | Wood:" + MountedArcher.WOOD_COST + " | Gold:" + MountedArcher.GOLD_COST + " | Training time: " + MountedArcher.SPAWN_TIME;
                    }
                }); 
    ...
    public void spawnUnits() {
        if (currentSpawn != null){ //Currently spawning something
            ...
            if (owner.getAge() == 1) {
                if (currentSpawn instanceof Knight){
                    counter = Knight.SPAWN_TIME;
                } else if (currentSpawn instanceof Lancer){
                    counter = Lancer.SPAWN_TIME;
                } else if (currentSpawn instanceof MountedArcher){
                    counter = MountedArcher.SPAWN_TIME;
                }
    ...
    public void spawnKnight() {
        ...
        if (owner.getMaxPopulation() > owner.getPopulation() && owner.getFood() >= Knight.FOOD_COST 
        && owner.getWood() >= Knight.WOOD_COST && owner.getGold() >= Knight.GOLD_COST) {
            //Consumes resources
            owner.changeFood((Game)getWorld(), -Knight.FOOD_COST);
            owner.changeWood((Game)getWorld(), -Knight.WOOD_COST);
            owner.changeGold((Game)getWorld(), -Knight.GOLD_COST);

            queue.add(new Knight(owner, spawnPoint, ((Game) getWorld()).getMap()));
            Game game = (Game)getWorld();
    ...
    /**
     * Spawns a MountedArcher
     */
    public void spawnMountedArcher() {        
        //Cannot spawn more Units if max population is reached, if there are not enough resources
        if (owner.getMaxPopulation() > owner.getPopulation() && owner.getFood() >= MountedArcher.FOOD_COST 
        && owner.getWood() >= MountedArcher.WOOD_COST && owner.getGold() >= MountedArcher.GOLD_COST) {
            //Consumes resources
            owner.changeFood((Game)getWorld(), -MountedArcher.FOOD_COST);
            owner.changeWood((Game)getWorld(), -MountedArcher.WOOD_COST);
            owner.changeGold((Game)getWorld(), -MountedArcher.GOLD_COST);

            queue.add(new MountedArcher(owner, spawnPoint, ((Game) getWorld()).getMap()));
            Game game = (Game)getWorld();
            if (game.isMultiplayer() && !game.isHosting()) { //Bug Fix for clients
                Unit.IDCounter--;
            }
    ...
    private void updateButtonImage() {
        //For Knight
        if (owner.getMaxPopulation() <= owner.getPopulation() || owner.getFood() < Knight.FOOD_COST || owner.getWood() < Knight.WOOD_COST 
        || owner.getGold() < Knight.GOLD_COST) {
            knightButton.setTransparency(100);
        } else {
            knightButton.setTransparency(255);
        }
        ...
        //For Mounter Archer
        if (owner.getMaxPopulation() <= owner.getPopulation() || owner.getFood() < MountedArcher.FOOD_COST || owner.getWood() < MountedArcher.WOOD_COST 
        || owner.getGold() < MountedArcher.GOLD_COST) {
            mountedArcherButton.setTransparency(100);
        } else {
            mountedArcherButton.setTransparency(255);
        }
