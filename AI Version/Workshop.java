...
{   
    ...
    public LinkedList<Button> getSpecificButtons() {
        ...
        if (owner.getAge() == 1 && owner.getID() == ((Game)getWorld()).getUser().getID()) { //Medieval Age
            ...
            list.add(new Button(organGunButton){ //Spawn Organ Gun
                    public void click() {
                        spawnOrganGun();
                        updateButtonImage();
                    }
                    ...
                    public String getHoverInfo() {
                        return "Food: " + OrganGun.FOOD_COST + " | Wood:" + OrganGun.WOOD_COST + " | Gold:" + OrganGun.GOLD_COST + " | Training time: " + OrganGun.SPAWN_TIME;
                    }
                });
    ...
    public void spawnUnits() {
        if (currentSpawn != null){ //Currently spawning something
            ...
            if (owner.getAge() == 1) {
                ...
                } else if (currentSpawn instanceof OrganGun){
                    counter = OrganGun.SPAWN_TIME;
                }
     ...
     * Spawns a OrganGun
     */
    public void spawnOrganGun() {        
        if (owner.getFood() >= OrganGun.FOOD_COST &&  owner.getWood() >= OrganGun.WOOD_COST && owner.getGold() >= OrganGun.GOLD_COST) {
            owner.changeFood((Game)getWorld(), -OrganGun.FOOD_COST);
            owner.changeWood((Game)getWorld(), -OrganGun.WOOD_COST);
            owner.changeGold((Game)getWorld(), -OrganGun.GOLD_COST);                

            OrganGun organGun = new OrganGun(owner, spawnPoint, ((Game) getWorld()).getMap()); queue.add(organGun);
            Game game = (Game)getWorld();
    ...
    private void updateButtonImage() {
        ...
        //For Organ Gun
        if (owner.getFood() < OrganGun.FOOD_COST || owner.getWood() < OrganGun.WOOD_COST 
        || owner.getGold() < OrganGun.GOLD_COST) {
            organGunButton.setTransparency(100);
        } else {
            organGunButton.setTransparency(255);
        }
